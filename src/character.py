...
class Character:
    ...
    def get_input_prompt(self, user_input, history, top_k=2):
        ...
        prompt = f" Only reply one line, and then stop. Don't copy what other characters says in the script. \
                    ...
                    Instruction: Now respond what the user says as {self.chara_name} {self.sys_prompt} The chat history is \"{history}\". \
                    The following script are the context you can reference from: {chunk_sim} \
                    You should completly forget you are an AI assistant and talk like {self.chara_name}. Input: {user_input} Don't repeat what the line before. Output: {self.chara_name}:"
        return prompt